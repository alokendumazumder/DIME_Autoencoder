...
if __name__ == '__main__':
    ...
    with open("precision_recall.txt", 'a') as f:
        task, dataset = args.path_fake.split('/')[-2], args.path_fake.split('/')[-3]
        model = args.path_fake.split('/')[-1].split('_')
        if len(model) == 8:
            f.write(f"HUAE {dataset} {task} precision: {precision} recall: {recall}\n")
        else:
            f.write(f"{' '.join(model[9:])} {dataset} {task} precision: {precision} recall: {recall}\n")
