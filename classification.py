...
parser.add_argument('--nuc-lambda', type=int, help='Uniform Lambda', default=0)
...
def main(args):
    ...
    if args.vanilla == True:
        print("using simple AE")
        dict = torch.load(args.checkpoint + "/" + args.dataset + "/"
                        + 'vanilla_'+str(args.n), map_location=torch.device('cpu'))
    else:
        print("using LoRAE")
        dict = torch.load(args.checkpoint + "/" + args.dataset + "/"
